...
This module provides helpers to train and evaluate a logistic regression
classifier (either PyTorch-based or scikit-learn) on fixed features. It includes
convenience wrappers for training on train/(optional) validation sets and
computing standard classification metrics on a test set.

Adapted from public references:
  - SLIP evaluate_zeroshot
  - https://github.com/mbanani/lgssl/blob/df45bae647fc24dce8a6329eb697944053e9a8a0/lgssl/evaluation/linear_probe.py
"""
...
def eval_linear_probe(
    ...
    """Wrapper that calls "train_linear_probe" and "test_linear_probe".
    ...
    Returns:
        A tuple of (metrics, dump) where metrics is a dict of evaluation metrics
        and dump includes prediction arrays and the trained weights.
    """
    if verbose:
...
def train_linear_probe(
    ...
    """Train the final linear classifier on train or train+valid."""
...
def test_linear_probe(
    ...
    Returns:
        A tuple of (metrics, dump) that includes arrays for preds/probs/targets.
    """
    if verbose:
