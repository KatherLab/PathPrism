"""
Minimal PyTorch logistic regression used for linear probing.
...
class LogisticRegression:
    def __init__(self, C, max_iter, verbose, random_state, **kwargs):
        ...
        Args:
            C: Inverse regularization strength (larger -> weaker regularization).
            max_iter: Max iterations for optimizer.
            verbose: Whether to print train-time losses.
    ...
    def compute_loss(self, feats, labels):
        """Compute cross-entropy loss plus L2 penalty on weights."""
    ...
    def fit(self, feats, labels):
        """Fit the linear layer using L-BFGS with strong Wolfe line search."""
    ...
    def load_weights(self, weight_path, feat_dim, num_classes):
        """Load weights into a fresh linear layer for inference/evaluation."""
        self.logreg = torch.nn.Linear(feat_dim, num_classes, bias=True)
        if weight_path is not None:
            self.logreg.load_state_dict(torch.load(weight_path, weights_only=False))
        self.logreg = self.logreg.to(self.device)

    def predict(self, feats):
        """Return (probs, preds) for input features."""
        feats = feats.to(self.device)
        with torch.no_grad():  
